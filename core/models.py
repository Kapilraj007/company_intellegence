...
class ValidationReport(BaseModel):
    company_name:     str
    total_expected:   int = EXPECTED_FIELD_COUNT
    total_received:   int
    total_passed:     int
    total_missing:    int
    total_failed:     int
    ...
    def print_report(self):
        ...
        print(f"\n  📋 FULL {EXPECTED_FIELD_COUNT}-FIELD DETAIL:")
        ...
        print(f"\n{sep}")
        print(
            f"  ✅ {self.total_passed}/{EXPECTED_FIELD_COUNT} validated"
...
def validate_golden_record(
    ...
    """
    # Backward compatibility: accept flat dict payload and normalize to rows.
    if isinstance(raw_data, dict):
        raw_data = flat_to_row_list(raw_data)
    ...
    for id_ in range(1, EXPECTED_FIELD_COUNT + 1):
    ...
    report = ValidationReport(
        ...
        total_failed=failed,
        completeness_pct=round((passed / EXPECTED_FIELD_COUNT) * 100, 1),
        results=results,
...
# ── Flat-object adapters ──────────────────────────────────────────────────────

def flat_to_row_list(flat: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Convert a flat golden record dict { snake_key: value } into the
    row-list format that validate_golden_record() expects.

    Each row: { "ID": int, "Category": str, "A/C": str,
                "Parameter": str, "Research Output / Data": str }

    Skips keys not found in _FLAT_KEY_TO_ID (internal/metadata keys).
    Missing IDs get a placeholder row so validation can report them as MISSING.
    """
    from core.prompts import _FLAT_KEY_TO_ID

    # Build id -> value from the flat dict
    ...
    for key, value in flat.items():
        ...
        if id_ is not None:
            ...
            id_to_value[id_] = text or "Not Found"

    # Build row list aligned to EXPECTED_PARAMETERS
    ...
    for id_, (cat, param, ac) in EXPECTED_PARAMETERS.items():
        rows.append({
            ...
            "A/C":                    ac,
            "Parameter":              param,
            "Research Output / Data": id_to_value.get(id_, "Not Found"),
        })
    return rows


def row_list_to_flat(rows: List[Dict[str, Any]]) -> Dict[str, str]:
    """Convert row-list format back to canonical flat object keys."""
    ...
    for row in rows:
        ...
        if value is None:
            value = row.get("Data")
        text = str(value).strip() if value is not None else ""
        flat[key] = text or "Not Found"
    return flat


class GoldenRecordFlat(BaseModel):
    """
    Canonical single-object schema validated by Pydantic:
    one dict containing exactly the 163 schema keys.
    """
    ...
    @classmethod
    def normalize_flat_values(cls, value: Any) -> Dict[str, str]:
        from core.prompts import _FLAT_KEY_TO_ID

        if isinstance(value, list):
            value = row_list_to_flat(value)
        if not isinstance(value, dict):
            raise ValueError("golden record must be a JSON object keyed by flat schema names")

        normalized: Dict[str, str] = {}
        for key in _FLAT_KEY_TO_ID:
            raw = value.get(key, "Not Found")
...
def validate_flat_golden_record(flat: Dict[str, Any], company_name: str) -> tuple:
    """
    Entry point for Agent 3 — accepts the new flat dict format.
    Converts to row list, runs full Pydantic validation,
    then converts valid rows BACK to flat dict before returning.

    Returns: (valid_flat: dict, report: ValidationReport)
    """
    canonical = GoldenRecordFlat.model_validate({"values": flat}).values
    row_list = flat_to_row_list(canonical)
    valid_rows, report = validate_golden_record(row_list, company_name)
    valid_flat = row_list_to_flat(valid_rows)
    return valid_flat, report
