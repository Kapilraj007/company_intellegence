...
def route_after_tests(state: Dict[str, Any]) -> str:
    ...
    max_rounds_raw = os.getenv("AGENT4_MAX_REMEDIATION_ROUNDS", "2").strip()
    try:
        max_rounds = max(0, int(max_rounds_raw))
    except ValueError:
        max_rounds = 2
    retry_count = int(state.get("pytest_retry_count", 0) or 0)
    if max_rounds == 0:
        print("[Agent4 Router] Remediation disabled (AGENT4_MAX_REMEDIATION_ROUNDS=0) → END")
        return "end"
    # Allow one retry when retry_count == max_rounds; stop only after exceeding cap.
    if retry_count > max_rounds:
        print(
            f"[Agent4 Router] Exceeded remediation cap "
            f"({retry_count}/{max_rounds}) → END"
        )
        return "end"
...
def run_tests(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    user_id = require_user_id(state.get("user_id"), context="Agent4 test runner")
    ...
    failed_ids = results["failed_parameter_ids"]
    if failed_ids:
        try:
            from core.local_store import get_local_store_client

            store = get_local_store_client()
            store.mark_parameters_as_failed(
                run_id=str(state.get("run_id", "")),
                company_id=str(state.get("company_id", "")),
                failed_param_ids=failed_ids,
                user_id=user_id,
            )
        except Exception as e:
            print(f"[Agent4] Warning: failed to mark params in local store (non-fatal): {e}")
