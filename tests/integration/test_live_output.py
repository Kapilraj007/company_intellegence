...
import pytest
from core.models import validate_golden_record, EXPECTED_PARAMETERS
...
def live_by_id(live_record):
    return {int(r["ID"]): r for r in live_record}


@pytest.fixture(scope="module")
def live_report(live_record):
    company = os.environ.get("LIVE_COMPANY_NAME", "Pipeline Output")
    _, report = validate_golden_record(live_record, company)
    return report
...
class TestLivePipelineOutput:
    ...
    def test_163_rows_present(self, live_record):
        assert len(live_record) == 163, \
            f"Expected 163 rows, got {len(live_record)}"

    def test_no_duplicate_ids(self, live_record):
        ids  = [int(r["ID"]) for r in live_record]
        seen, dupes = set(), set()
        for i in ids:
            (dupes if i in seen else seen).add(i)
        assert not dupes, f"Duplicate IDs in live output: {sorted(dupes)}"

    def test_all_ids_in_range(self, live_record):
        for row in live_record:
            id_ = int(row["ID"])
    ...
    def test_no_raw_sentinel_values_in_output(self, live_record):
        ...
        for row in live_record:
            data = row.get("Research Output / Data", "").strip().lower()
            assert data not in raw_sentinels, \
                f"Raw sentinel '{data}' survived in ID={row['ID']} ({row.get('Parameter')})"

    # ── Structure ─────────────────────────────────────────────────────────
    def test_all_rows_have_required_keys(self, live_record):
        required = {"ID", "Category", "A/C", "Parameter", "Research Output / Data"}
        for row in live_record:
            missing = required - set(row.keys())
            assert not missing, \
                f"ID={row.get('ID')} missing keys: {missing}"

    def test_all_ac_values_valid(self, live_record):
        for row in live_record:
            assert row.get("A/C") in ("Atomic", "Composite"), \
                f"ID={row.get('ID')}: invalid A/C='{row.get('A/C')}'"